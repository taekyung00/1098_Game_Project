...
Boots::Boots(Math::ivec2 start_index, ItemKind item_kind, UseItem use_item, UseItemRank use_item_rank) :
	Item(start_index, item_kind, use_item, use_item_rank),
	cost_texture(nullptr)
{

	AddGOComponent(new CS230::Sprite("Assets/Boots.spt", this));
	is_get = false;
...
void Boots::ResolveCollision(GameObject* other_object) {
...
void Boots::Draw([[maybe_unused]] Math::TransformationMatrix camera_matrix) {
	if (is_get == false) {
