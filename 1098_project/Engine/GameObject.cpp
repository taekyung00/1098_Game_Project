...
CS230::GameObject::GameObject(Math::ivec2 index) :
    GameObject(index, 0, {1,1})
{
}
...
CS230::GameObject::GameObject(Math::ivec2 index, double rotation, Math::vec2 scale):
    velocity({ 0,0 }),
    index(index),
    scale(scale),
    position({ start_position.x + index.x * tile_size.x * scale_const.x, start_position.y + index.y * tile_size.y * scale_const.y  }),
    rotation(rotation),
    current_state(&state_none),
    matrix_outdated(true),
    destroy(false)
...
{
    return index == other_object->index;
...
const Math::ivec2 CS230::GameObject::GetIndex() const
{
    return index;
}

Math::ivec2& CS230::GameObject::SetIndex()
{
    return index;
}
