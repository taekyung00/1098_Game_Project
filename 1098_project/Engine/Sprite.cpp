...
CS230::Sprite::Sprite(const std::filesystem::path& sprite_file, GameObject* given_object, bool just_divide)
{
    if (just_divide) {
        Load(sprite_file, given_object,just_divide);
    }
}
...
void CS230::Sprite::Load(const std::filesystem::path& sprite_file, GameObject* _given_object, bool just_divide)
{
    given_object = _given_object;
    animations.clear();
    if (sprite_file.extension() != ".spt") {
...
void CS230::Sprite::Draw(Math::TransformationMatrix display_matrix, int texel)
{
    texture->Draw(display_matrix * Math::TranslationMatrix(-GetHotSpot(0)), GetFrameTexel(texel), GetFrameSize());
}
